...
def flag_anomalies(amounts: list[int], student_id: str) -> list[int]:
    ...
    print("the Sample it Calculated", sample)

    # 3️⃣ Statistical threshold --------------------------------------------
    μ = mean(sample)
    σ = stdev(sample) if len(sample) > 1 else 0 # σ=0 if sample size = 1
    print("the mean it Calculated", μ )
    print("the stdev it Calculated", σ )
...
transactions = [102, 98, 97, 99, 101, 100, 250,
    97, 95, 420, 101, 99]
result = flag_anomalies(transactions, student_id="9110715") #Ubaid Ullah Student ID
print(result)
